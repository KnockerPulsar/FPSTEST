...
public class SpeedLinesScript : MonoBehaviour
{
    ...
    public float thresholdVelocity;          //The threshold velocity after which the speedlines appear.
    ...
    void Start()
    {
        ...
        thresholdVelocity = pVars.maxVelocity * multiplier;
        speedLines.Stop();
    }
    ...
    void Update()
    {
        if (pVars.playerRB.velocity.magnitude > thresholdVelocity)
            speedLines.Play();
        else
            speedLines.Stop();
