...
public class KatanaGrappleScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        wepSys = PlayerVariables.player.GetComponent<WeaponSystemScript>();
    ...
    void Update()
    {
        ...
        {
            transform.rotation = Quaternion.Slerp(transform.rotation, PlayerVariables.playerCam.transform.rotation * baseGunRotation, GrappleSlerpVal);
            if (GrappleSlerpVal >= 1 && katanaAnimator.enabled == false)
            {
                print("Animator activated");
                katanaAnimator.enabled = true;
                GetComponent<BaseWeaponScript>().currentWeaponState = (int)WeaponState.idle;
            }
        }
    ...
    {
        if (!PlayerVariables.isGrappling && GrappleSlerpVal < 1f)
        {
    ...
    private void Grapple()
    {
        ...
        if (wepSys.hasWeapon[(int)WeaponCodes.katana] && wepSys.currentWeaponScript.currentWeaponState == (int)WeaponState.idle)
        {
            ...
            hits = Physics.RaycastAll(PlayerVariables.playerCam.transform.position, PlayerVariables.playerCam.transform.forward.normalized, grappleRange);
            if (hits.Length < 1) return;

            wepSys.currentWeaponScript.currentWeaponState = (int)WeaponState.grappling;
            katanaAnimator.enabled = false;
            PlayerVariables.isGrappling = true;
            line.enabled = true;
            grappleOrigin = grappleOriginObj.transform.position;

            grapplePoint = hits[0].point;                           //Finds the first intersection
            joint = PlayerVariables.player.AddComponent<SpringJoint>();       //Adds a spring joint to the player
    ...
    private void UpdateGrappleLocation()
    {
        ...
        transform.forward = Vector3.Slerp(transform.forward, -dir, Time.deltaTime * PlayerVariables.playerRB.velocity.magnitude / 2f);
    ...
    private void EndGrapple()
    {
        if (wepSys.currentWeaponScript)
        {
            ...
            line.enabled = false;
            PlayerVariables.isGrappling = false;
            Destroy(joint);
            line.enabled = false;
    ...
    {
        RaycastHit[] hit = Physics.RaycastAll(PlayerVariables.playerCam.transform.position, PlayerVariables.playerCam.transform.forward, grappleRange);
        float dist = 151f;
        if (hit.Length > 0)
            dist = Vector3.Distance(hit[0].point, PlayerVariables.playerCam.transform.position);
        ...
        {
            if ((dist > grappleRange || dist < minFeedbackDist ) && !PlayerVariables.isGrappling)
                Destroy(spawnedObj);
            else
            {
                if (PlayerVariables.isGrappling)
                {
