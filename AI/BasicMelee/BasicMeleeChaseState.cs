...
public class ChaseState : StateInterface
{
    ...
    public ChaseState(GameObject inputEntity, float inputMS, float inputSD, float inputJCI, Rigidbody inputRB, GameObject inputCheckObj, PlayerVariables inputPvars)
    {
        pVars = inputPvars;
        entity = inputEntity;
    ...
    public void Tick()
    {
        ...
        if (HVelocityMag < 1)
        {
            ...
            if (jumpCheckBottom.Length > 0 && !jumpCheckBottom[0].collider.CompareTag("Player"))
            {
                ...
                //If so, sets the boolean an breaks out of the loop.
