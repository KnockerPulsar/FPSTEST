...
public class BasicMelee : MonoBehaviour
{
    ...
    private IdleState idleState;                            //The idle behaviour of the enemy. Ticks only when the player is out of range/not seen by the enemy.
    private ChaseState chaseState;                          //The chase behaviour of the enemy. Ticks when the player is in range but not in stoppingDist.
    private AttackState attackState;                        //The attack behaviour of the enemy. Ticks when the player is closer than the stopping distance.
    ...
    void Start()
    {
        ...
        idleState = new IdleState(gameObject, idleFloatingCurve, pVars);
        chaseState = new ChaseState(gameObject, movementSpeed, stoppingDist, jumpCheckIterations, RB, checkObj, pVars);
        attackState = new AttackState(beforeAttackDelay, rotationDuration, particleDelay, afterAttackDelay, gameObject, RB, smile, poker, attackPS, pVars);
